...
import { PyrightLanguageProvider } from "../python/PyrightLanguageProviderBrowser";
...
const _pyrightLanguageProvider: PyrightLanguageProvider =
  new PyrightLanguageProvider(
...
connection.onHover(async (params, token) => {
  return await dispatch(params, {
    async sas(languageService) {
      return await languageService.completionProvider.getHelp(params.position);
    },
...
connection.onDocumentHighlight(
  async (params: DocumentHighlightParams, token: CancellationToken) => {
    return await dispatch(params, {
      async python(pyrightLanguageService) {
        return await pyrightLanguageService.onDocumentHighlight(params, token);
      },
    });
  },
);
