...
var SyntaxColor = function (model) {
  var self = this,
    parsingQueue = [],
    parsingState = 2, //LanguageService.ParsingState.ENDED,
    syntaxTable = [],
    lexer = new SasLexerEx(model),
    lastToken = null,
    currTokenIndex = 0,
    parsedRange = {},
  ...
  function _startParse(change) {
    ...
    if (parsedRange.startLine === parsedRange.endLine && tmpSyntaxLine) {
      while (tmpSyntaxLine[i] && tmpSyntaxLine[i].start < parsedRange.endCol) {
        i++;
      }
    ...
    if (token.end.line !== endLine) {
      ...
      do {
        ...
        if (currTokenIndex < syntaxLine.length) {
          syntaxLine.splice(currTokenIndex, syntaxLine.length - currTokenIndex); //clear the garbages
        }
        //add end marks
